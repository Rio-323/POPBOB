...
$(document).ready(function() {
    ...
    $('#floatingId').keyup(function() {
        ...
        if (id != '') {
            if (koreanRegExp.test(id)) {
                ...
                $.ajax({
                    ...
                    success: function(result) { 
                        if (result == 0) {  
                            $('#id_result').text('사용 가능한 아이디').css('color', '#76daaf');
                        } else if (result == 1) {
            ...
            $('#id_result').text('아이디를 입력해주세요!').css('color','red');
        }
    });
    ...
    $('#floatingPassword').keyup(function() {
    	...
    	if (password != '') {
            if (!passRegExp.test(password)) {
            	...
            	$('#password_result').text('사용가능한 비밀번호').css('color', '#76daaf');
            } 
    	} else {
        	$('#password_result').text('비밀번호를 입력해주세요!').css('color','red');
        }
    })
    ...
    $('#check_floatingPassword').keyup(function() {
    	...
    	if(check_password != '') {
    		if(check_password != password) {
        		...
        		$('#password_check_result').text('비밀번호 확인').css('color', '#76daaf');
        	}
    	} else {
        	$('#password_check_result').text('비밀번호 확인을 입력해주세요!').css('color','red');
        }
    ...
    $('#floatingEmail').keyup(function() {
        ...
        if (email != '') {
            if (!emailRegExp.test(email)) {
                ...
                $.ajax({
                    ...
                    success: function(result) { 
                    	console.log(result);
                        if (result == 0) {  
                            $('#email_result').text('사용 가능한 이메일').css('color', '#76daaf');
                        } else if (result == 1) {
            ...
            $('#email_result').text('이메일을 입력해주세요 !').css('color','red');
        }
    });
      ...
      <div class="col-sm-9 col-md-7 col-lg-5 mx-auto" >
          <div class="card-body p-4 p-sm-5">
            ...
            <form action="signup_ok.do" method="post" name="sfrm" >
              ...
              <div class="d-grid">
                <button class="btn btn-login text-uppercase fw-bold" type="submit" id="btn-signup" style="background:#13538a"><span style="color:white;">Sign Up</span></button>
              </div>
              <hr class="my-4">
